...
import 'astro_list_tile_theme.dart';
...
/// Defines the title font used for [AstroListTile] descendants of a [ListTileTheme].
///
...
enum AstroListTileStyle {
  /// Use a title font that's appropriate for a [AstroListTile] in a list.
  list,

  /// Use a title font that's appropriate for a [AstroListTile] that appears in a [Drawer].
  drawer,
}

/// Where to place the control in widgets that use [AstroListTile] to position a
/// control next to a label.
///
/// See also:
///
///  * [CheckboxListTile], which combines a [AstroListTile] with a [Checkbox].
///  * [RadioListTile], which combines a [AstroListTile] with a [Radio] button.
///  * [SwitchListTile], which combines a [AstroListTile] with a [Switch].
///  * [ExpansionTile], which combines a [AstroListTile] with a button that expands
///    or collapses the tile to reveal or hide the children.
enum AstroListTileControlAffinity {
...
/// Defines how [AstroListTile.leading] and [AstroListTile.trailing] are
/// vertically aligned relative to the [AstroListTile]'s titles
/// ([AstroListTile.title] and [AstroListTile.subtitle]).
///
/// See also:
///
///  * [AstroListTile.titleAlignment], to configure the title alignment for an
///    individual [AstroListTile].
///  * [ListTileThemeData.titleAlignment], to configure the title alignment
///    for all of the [AstroListTile]s under a [ListTileTheme].
///  * [ThemeData.listTileTheme], to configure the [ListTileTheme]
///    for an entire app.
enum AstroListTileTitleAlignment {
  /// The top of the [AstroListTile.leading] and [AstroListTile.trailing] widgets are
  /// placed [AstroListTile.minVerticalPadding] below the top of the [AstroListTile.title]
  /// if [AstroListTile.isThreeLine] is true, otherwise they're centered relative
  /// to the [AstroListTile.title] and [AstroListTile.subtitle] widgets.
  ///
  /// This is the default when [ThemeData.useMaterial3] is true.
  threeLine,

  /// The tops of the [AstroListTile.leading] and [AstroListTile.trailing] widgets are
  /// placed 16 pixels below the top of the [AstroListTile.title] widget,
  /// if the [AstroListTile]'s overall height is greater than 72, otherwise the
  /// [AstroListTile.trailing] widget is centered relative to the [AstroListTile.title] and
  /// [AstroListTile.subtitle] widgets, and the [AstroListTile.leading] widget is 16 pixels
  /// below the top of [AstroListTile.title], or center-aligned with [AstroListTile.title],
  /// whichever makes the [AstroListTile.leading] closer to the top edge of [AstroListTile.title].
  ///
  ...
  /// The [AstroListTile.leading] and [AstroListTile.trailing] widgets are
  /// centered relative to the [AstroListTile]'s titles.
  center,

  /// The bottoms of the [AstroListTile.leading] and [AstroListTile.trailing] widgets are
  /// placed [AstroListTile.minVerticalPadding] above the bottom of the [AstroListTile]'s
  /// titles.
  bottom;
  ...
  double _yOffsetFor(double childHeight, double tileHeight, _RenderListTile listTile, bool isLeading) {
    return switch (this) {
      AstroListTileTitleAlignment.threeLine =>
        listTile.isThreeLine
            ? AstroListTileTitleAlignment.top._yOffsetFor(childHeight, tileHeight, listTile, isLeading)
            : AstroListTileTitleAlignment.center._yOffsetFor(childHeight, tileHeight, listTile, isLeading),
      ...
      AstroListTileTitleAlignment.titleHeight when tileHeight > 72.0 => 16.0,
      // For smaller tiles, trailing should always be centered. Leading can be
      // centered or closer to the top. It should never be further than 16dp
      // to the top.
      AstroListTileTitleAlignment.titleHeight =>
        isLeading ? math.min((tileHeight - childHeight) / 2.0, 16.0) : (tileHeight - childHeight) / 2.0,
      AstroListTileTitleAlignment.top => listTile.minVerticalPadding,
      AstroListTileTitleAlignment.center => (tileHeight - childHeight) / 2.0,
      AstroListTileTitleAlignment.bottom => tileHeight - childHeight - listTile.minVerticalPadding,
    };
  }
}
...
/// are not painted by the [AstroListTile] itself but by the [Material] widget
...
/// {@end-tool}
///
/// ## Performance considerations when wrapping [AstroListTile] with [Material]
///
/// Wrapping a large number of [AstroListTile]s individually with [Material]s
/// is expensive. Consider only wrapping the [AstroListTile]s that require it
/// or include a common [Material] ancestor where possible.
///
/// [AstroListTile] must be wrapped in a [Material] widget to animate [tileColor],
...
///
/// {@tool dartpad}
/// This example showcases how [AstroListTile] needs to be wrapped in a [Material]
...
///
/// {@tool dartpad}
/// This sample shows the creation of a [AstroListTile] using [ThemeData.useMaterial3] flag,
...
///
/// {@tool dartpad}
/// This sample shows [AstroListTile]'s [textColor] and [iconColor] can use
/// [WidgetStateColor] color to change the color of the text and icon
/// when the [AstroListTile] is enabled, selected, or disabled.
///
/// ** See code in examples/api/lib/material/list_tile/list_tile.3.dart **
/// {@end-tool}
///
/// {@tool dartpad}
/// This sample shows [AstroListTile.titleAlignment] can be used to configure the
...
///
/// {@tool snippet}
/// To use a [AstroListTile] within a [Row], it needs to be wrapped in an
/// [Expanded] widget. [AstroListTile] requires fixed width constraints,
...
/// See also:
///
///  * [ListTileTheme], which defines visual properties for [AstroListTile]s.
///  * [ListView], which can display an arbitrary number of [AstroListTile]s
///    in a scrolling list.
///  * [CircleAvatar], which shows an icon representing a person and is often
///    used as the [leading] element of a ListTile.
///  * [Card], which can be used with [Column] to show a few [AstroListTile]s.
///  * [Divider], which can be used to separate [AstroListTile]s.
///  * [AstroListTile.divideTiles], a utility for inserting [Divider]s in between [AstroListTile]s.
///  * [CheckboxListTile], [RadioListTile], and [SwitchListTile], widgets
///    that combine [AstroListTile] with other controls.
///  * Material 3 [AstroListTile] specifications are referenced from <https://m3.material.io/components/lists/specs>
///    and Material 2 [AstroListTile] specifications are referenced from <https://material.io/design/components/lists.html>
...
class AstroListTile extends StatelessWidget {
  /// Creates a list tile.
  ///
  ...
  /// [AstroListTileStyle.list], [TextTheme.titleMedium] will be used and if ListTile style
  /// is [AstroListTileStyle.drawer], [TextTheme.bodyLarge] will be used.
  ...
  /// is also null then [AstroListTileStyle.list] is used.
  ///
  /// See also:
  ///
  /// * [ListTileTheme.of], which returns the nearest [ListTileTheme]'s
  ///   [ListTileThemeData].
  final AstroListTileStyle? style;

  /// The tile's internal padding.
  ///
  /// Insets a [AstroListTile]'s contents: its [leading], [title], [subtitle], and [trailing] widgets.
  ///
  ...
  /// The minimum width allocated for the [AstroListTile.leading] widget.
  ///
  ...
  /// The minimum height allocated for the [AstroListTile] widget.
  ///
  ...
  /// is used. If that is also null then [AstroListTileTitleAlignment.threeLine]
  /// is used.
  ///
  /// See also:
  ///
  /// * [ListTileTheme.of], which returns the nearest [ListTileTheme]'s
  ///   [ListTileThemeData].
  final AstroListTileTitleAlignment? titleAlignment;
  ...
  bool _isDenseLayout(ThemeData theme, AstroListTileThemeData tileTheme) {
    return dense ?? tileTheme.dense ?? theme.listTileTheme.dense ?? false;
  }

  Color _tileBackgroundColor(ThemeData theme, AstroListTileThemeData tileTheme, AstroListTileThemeData defaults) {
  ...
  Widget build(BuildContext context) {
    ...
    final AstroListTileThemeData tileTheme = AstroListTileTheme.of(context);
    final AstroListTileStyle listTileStyle = style ?? tileTheme.style ?? AstroListTileThemeData.defaults.style ?? AstroListTileStyle.list;
    final AstroListTileThemeData defaults = theme.useMaterial3
        ? _AstroLisTileDefaultsM3(context)
        : _AstroLisTileDefaultsM2(context, listTileStyle);
    final states = <WidgetState>{if (!enabled) WidgetState.disabled, if (selected) WidgetState.selected};
    ...
    final Color? effectiveIconButtonColor =
        effectiveIconColor ?? iconButtonTheme.style?.foregroundColor?.resolve(states) ?? defaultEffectiveIconColor;
    ...
    final iconButtonThemeData = IconButtonThemeData(
      style:
          IconButtonTheme.of(context).style?.copyWith(foregroundColor: WidgetStatePropertyAll<Color?>(effectiveIconButtonColor)) ??
    ...
    Widget? leadingIcon;
    if (leading != null) {
      leadingIcon = AnimatedDefaultTextStyle(style: leadingAndTrailingStyle!, duration: kThemeChangeDuration, child: leading!);
    }
    ...
    final Widget titleText = AnimatedDefaultTextStyle(style: titleStyle, duration: kThemeChangeDuration, child: title ?? const SizedBox());
    ...
    Widget? trailingIcon;
    if (trailing != null) {
      trailingIcon = AnimatedDefaultTextStyle(style: leadingAndTrailingStyle!, duration: kThemeChangeDuration, child: trailing!);
    }
    ...
    final mouseStates = <WidgetState>{if (!enabled || (onTap == null && onLongPress == null)) WidgetState.disabled};
    ...
    final AstroListTileTitleAlignment effectiveTitleAlignment =
        titleAlignment ??
        //tileTheme.titleAlignment ??
        (theme.useMaterial3 ? AstroListTileTitleAlignment.threeLine : AstroListTileTitleAlignment.titleHeight);

    final effectiveShape = shape ?? tileTheme.shape ?? RoundedRectangleBorder(borderRadius: BorderRadius.circular(8));
    ...
    return InkWell(
      ...
      hoverColor: hoverColor ?? theme.colorScheme.onSurface.withValues(alpha: 0.05),
      ...
      child: Semantics(
        ...
        enabled: enabled,
        child: Ink(
          decoration: ShapeDecoration(shape: effectiveShape, color: _tileBackgroundColor(theme, tileTheme, defaults)),
          child: SafeArea(
            top: false,
            bottom: false,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(FlagProperty('isThreeLine', value: isThreeLine, ifTrue: 'THREE_LINE', ifFalse: 'TWO_LINE', showName: true));
    properties.add(FlagProperty('dense', value: dense, ifTrue: 'true', ifFalse: 'false', showName: true));
    properties.add(DiagnosticsProperty<VisualDensity>('visualDensity', visualDensity, defaultValue: null));
    properties.add(DiagnosticsProperty<ShapeBorder>('shape', shape, defaultValue: null));
    properties.add(DiagnosticsProperty<AstroListTileStyle>('style', style, defaultValue: null));
    ...
    properties.add(DiagnosticsProperty<AstroListTileTitleAlignment>('titleAlignment', titleAlignment, defaultValue: null));
  }
}
...
class _RenderListTile extends RenderBox with SlottedContainerRenderObjectMixin<_ListTileSlot, RenderBox> {
  _RenderListTile({
    ...
    required AstroListTileTitleAlignment titleAlignment,
  }) : _isDense = isDense,
  ...
  AstroListTileTitleAlignment get titleAlignment => _titleAlignment;
  AstroListTileTitleAlignment _titleAlignment;
  set titleAlignment(AstroListTileTitleAlignment value) {
  ...
  double computeMinIntrinsicHeight(double width) {
    ...
    final double contentHeight = titleMinHeight + (subtitleMinHeight ?? 0.0) + topAndBottomPaddingMultiplier * _minVerticalPadding;
  ...
  _Sizes _computeSizes(
    ...
    final double titleStart = leadingSize == null ? 0.0 : math.max(_minLeadingWidth, leadingSize.width) + _effectiveHorizontalTitleGap;

    final double adjustedTrailingWidth = trailingSize == null ? 0.0 : math.max(trailingSize.width + _effectiveHorizontalTitleGap, 32.0);

    final BoxConstraints textConstraints = looseConstraints.tighten(width: tileWidth - titleStart - adjustedTrailingWidth);
    ...
    if (subtitle == null) {
      ...
      final double targetSubtitleY =
          (isThreeLine ? (isDense ? 42.0 : 48.0) : (isDense ? 48.0 : 52.0)) + visualDensity.vertical * 2.0 - subtitleBaseline;
    ...
    if (positionChild != null) {
      ...
      if (leading != null && leadingSize != null) {
        positionChild(
          leading,
          Offset(isLTR ? 0.0 : tileWidth - leadingSize.width, titleAlignment._yOffsetFor(leadingSize.height, tileHeight, this, true)),
        );
      }
      ...
      if (trailing != null && trailingSize != null) {
        positionChild(
          trailing,
          Offset(isLTR ? tileWidth - trailingSize.width : 0.0, titleAlignment._yOffsetFor(trailingSize.height, tileHeight, this, false)),
        );
      }
    }
...
class _AstroLisTileDefaultsM2 extends AstroListTileThemeData {
  _AstroLisTileDefaultsM2(this.context, AstroListTileStyle style)
    : super(
  ...
  TextStyle? get titleTextStyle => switch (style!) {
    AstroListTileStyle.drawer => _textTheme.bodyLarge,
    AstroListTileStyle.list => _textTheme.titleMedium,
  };

  @override
  TextStyle? get subtitleTextStyle => _textTheme.bodyMedium!.copyWith(color: _textTheme.bodySmall!.color);
...
// dart format off
class _AstroLisTileDefaultsM3 extends AstroListTileThemeData {
  _AstroLisTileDefaultsM3(this.context)
    : super(
        contentPadding: const EdgeInsetsDirectional.symmetric(horizontal: 12.0),
        minLeadingWidth: 24,
